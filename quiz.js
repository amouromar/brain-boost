...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function buildQuiz() {
    ...
    if (!selectedCategory || !selectedDifficulty) {
      // Display a message or take appropriate action to indicate that selections are required
      return;
    }
  ...
  function fetchQuizData(category) {
    const selectedDifficulty =
      document.getElementById("difficultyDropdown").value;
    const apiUrl = category
      ? `https://opentdb.com/api.php?amount=10&category=${category}&difficulty=${selectedDifficulty}&type=multiple`
  ...
  function updateProgressBar() {
    ...
    // If it's the last question, indicate that the quiz is complete
